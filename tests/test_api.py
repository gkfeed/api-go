...
def test_basic_credentials_are_rejected_by_protected_routes(client: TestClient) -> None:
    response = client.get("/api/v2/list", auth=("reader", "secret"))

    assert response.status_code == 401


def test_login_rejects_invalid_password(client: TestClient) -> None:
    response = client.post("/api/v2/auth/login", json={"username": "reader", "password": "wrong"})

    assert response.status_code == 401

    unknown = client.post("/api/v2/auth/login", json={"username": "missing", "password": "wrong"})
    assert unknown.status_code == 401


def test_login_refresh_and_logout(client: TestClient) -> None:
    login = client.post("/api/v2/auth/login", json={"username": "reader", "password": "secret"})
    ...
    assert (
        client.get(
            "/api/v2/list", headers={"Authorization": f"Bearer {tokens['access_token']}"}
        ).status_code
        == 200
    )

    refreshed = client.post("/api/v2/auth/refresh", json={"refresh_token": tokens["refresh_token"]})
    assert refreshed.status_code == 200
    rotated = refreshed.json()
    assert rotated["refresh_token"] != tokens["refresh_token"]

    reused = client.post("/api/v2/auth/refresh", json={"refresh_token": tokens["refresh_token"]})
    assert reused.status_code == 401
    assert (
        client.get(
            "/api/v2/list", headers={"Authorization": f"Bearer {rotated['access_token']}"}
        ).status_code
        == 401
    )
...
def test_logout_all_revokes_access_token(authenticated_client: TestClient) -> None:
    assert authenticated_client.post("/api/v2/auth/logout-all").status_code == 204
    assert authenticated_client.get("/api/v2/list").status_code == 401
...
def test_add_and_list_feed(authenticated_client: TestClient) -> None:
    ...
    assert authenticated_client.get("/api/v2/list").json() == [response.json()["item"]]


def test_add_rejects_unknown_fields(authenticated_client: TestClient) -> None:
    response = authenticated_client.post(
        "/api/v2/add", json={"title": "Example", "unexpected": True}
    )
...
def test_add_lazy_infers_and_normalizes_feed(authenticated_client: TestClient) -> None:
    response = authenticated_client.post(
        "/api/v2/add_lazy", json={"url": "https://tok.adminforge.de/@example"}
    )
...
def test_add_lazy_rejects_invalid_url(authenticated_client: TestClient) -> None:
    response = authenticated_client.post(
        "/api/v2/add_lazy", json={"url": "https://example.com/feed"}
    )
...
def test_delete_only_deletes_the_authenticated_users_feed(
    ...
    assert authenticated_client.delete("/api/v2/delete?id=1").status_code == 404
    assert authenticated_client.get("/api/v2/delete?id=1").status_code == 404
...
def test_items_are_paginated_and_deleted_items_are_hidden(
    ...
    first_page = authenticated_client.get("/api/v2/get_items?limit=2")
    assert first_page.status_code == 200
    assert [item["id"] for item in first_page.json()["items"]] == [3, 2]
    assert first_page.json()["next_cursor"] == 2

    second_page = authenticated_client.get("/api/v2/get_items?limit=2&cursor=2")
    assert [item["id"] for item in second_page.json()["items"]] == [1]
    assert "next_cursor" not in second_page.json()

    deleted = authenticated_client.post("/api/v2/add_deleted_items", json={"itemIds": [3]})
    assert deleted.status_code == 200
    assert deleted.content == b""
    assert [
        item["id"] for item in authenticated_client.get("/api/v2/get_items").json()["items"]
    ] == [2, 1]
